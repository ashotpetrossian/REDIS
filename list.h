...
public:
    ...
    // T& back() const {
    //     if (this->empty()) { throw std::out_of_range("The List is empty\n"); }
    //     return tail->data;
    // }
    ...
    bool insert(unsigned int pos, const T& value) { // modified to bool
        if (pos > m_size || pos < 0) { 
            return false; 
        } else if (pos == 0) { 
    ...
    void sort() {
        ...
        List<T> tmp(*this);
        while (size != 0) {
            node<T>* curr_head_tmp = tmp.head;
            T min = tmp.front();
            unsigned int pos{0};
            while (curr_head_tmp != nullptr) {
                if (curr_head_tmp->data < min) {
                    min = curr_head_tmp->data;
                }
                curr_head_tmp = curr_head_tmp->next;
            }
            curr_head_tmp = tmp.head;
            while (curr_head_tmp != nullptr) {
                if (curr_head_tmp->data == min) {
                    break;
                }
